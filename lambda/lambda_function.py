...
class LaunchRequestOrPlayAudioHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        if is_request_type("LaunchRequest")(handler_input):
            ...
            handler_input.response_builder.speak(
                _(data.WELCOME_MSG).format(
                    data.home["card"]["title"])
            ).set_should_end_session(False)
            return handler_input.response_builder.response
        
        else:
            print('023958_PLAY_AUDIO')
            return util.play(url=util.audio_data(handler_input)["url"],
                            offset=0,
                            text=_(data.NOW_PLAYING).format(
                                util.audio_data(handler_input)["card"]["title"]),
                            card_data=util.audio_data(handler_input)["card"],
...
class ResumeIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        speech = _(data.RESUME_MSG).format(
            util.audio_data(handler_input)["card"]["title"])
        return util.play(
            url=util.audio_data(handler_input)["url"], offset=0,
            text=speech, card_data=util.audio_data(handler_input)["card"],
            response_builder=handler_input.response_builder)
...
class PlaybackNearlyFinishedHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        return util.play_later(
            url=util.audio_data(handler_input)["url"],
            card_data=util.audio_data(handler_input)["card"],
            response_builder=handler_input.response_builder)
...
class PlaybackFailedHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        return util.play(
            url=util.audio_data(handler_input)["url"], offset=0, text=None,
            card_data=None,
            response_builder=handler_input.response_builder)
