...
Author: Kia Mei Somabes [kia.somabes@gmail.com]
"""
...
class LaunchRequestOrPlayAudioHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        return util.play(url=util.audio_data(request)["station"],
                         offset=0,
...
class ResumeIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        speech = _(data.RESUME_MSG).format(
            util.audio_data(request)["card"]["title"])
        return util.play(
            url=util.audio_data(request)["station"], offset=0,
...
class PlaybackNearlyFinishedHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        return util.play_later(
            url=util.audio_data(request)["station"],
            card_data=util.audio_data(request)["card"],
            response_builder=handler_input.response_builder)
...
class PlaybackFailedHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        return util.play(
            url=util.audio_data(request)["station"], offset=0, text=None,
            card_data=None,
            response_builder=handler_input.response_builder)
